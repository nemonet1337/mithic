...
use mithic_i18n::{I18n, DEFAULT_LOCALE};
...
impl RequestLocale {
    pub fn new(locale: String) -> Self { Self { locale } }
    pub fn as_str(&self) -> &str { &self.locale }
}

impl Default for RequestLocale {
    fn default() -> Self { Self { locale: DEFAULT_LOCALE.to_string() } }
}
...
pub async fn locale_middleware(mut request: Request, next: Next) -> Response {
    ...
    request.extensions_mut().insert(RequestLocale::new(locale.to_string()));
