...
最終更新: 2026-08-25（AP inbox Delete/Update/Accept/Reject/Block）
...
- [x] WebSocket で note / notification 受信
...
| **ピン留め** | バックエンド pin/unpin API あり | フロントは「準備中」トースト。プロフィールでのピン表示未確認 |
| **ブロック / ミュート UI** | API は実装済み | フロント PostCard メニューが「準備中」 |
...
- [ ] フロント「設定」の他セクション（プライバシー、ミュート一覧、ブロック一覧等）
...
2. **フロント: ピン / ブロック / ミュートを API に接続**  
   既に API があるのに UI が準備中。
