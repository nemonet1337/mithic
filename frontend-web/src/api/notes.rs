...
pub async fn fetch_replies(token: &str, id: &str) -> Result<Vec<Note>, ApiError> {
    request::<Vec<Note>, ()>("GET", &format!("v1/notes/{}/replies", id), Some(token), None).await
}
...
pub async fn add_reaction(token: &str, note_id: &str, emoji: &str) -> Result<(), ApiError> {
    #[derive(Serialize)]
    struct Body<'a> { emoji: &'a str }
    request::<(), Body>("POST", &format!("v1/notes/{}/reactions", note_id), Some(token), Some(&Body { emoji })).await
}

pub async fn remove_reaction(token: &str, note_id: &str, emoji: &str) -> Result<(), ApiError> {
    request::<(), ()>("DELETE", &format!("v1/notes/{}/reactions/{}", note_id, emoji), Some(token), None).await
}

pub async fn renote(token: &str, note_id: &str) -> Result<Note, ApiError> {
    request::<Note, ()>("POST", &format!("v1/notes/{}/renotes", note_id), Some(token), None).await
}
