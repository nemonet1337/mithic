...
use crate::db::queries::{create_note, create_notification, get_actor_by_id, get_note_by_id};
use crate::extract::extract_hashtags;
use crate::extract::extract_local_mentions;
...
pub async fn create_note_service(
    ...
    if created.visibility == NoteVisibility::Public {
        ...
        tokio::spawn(async move {
            crate::db::cache::invalidate_public_timelines(&dragonfly).await;
        });
    }
...
async fn spawn_note_notifications(state: AppState, author: Actor, created: Note, dto: NoteDto) {
    ...
    if let Some(ref text) = created.text {
        ...
        if !names.is_empty() {
            ...
            let actors: Vec<Actor> = match crate::db::queries::rows_to(rows) {
                Ok(a) => a,
                Err(e) => {
