...
use crate::models::note::{Note, NoteId, NoteVisibility};
...
async fn ensure_note_visible(
    state: &AppState,
    note: &Note,
    viewer: Option<crate::models::actor::ActorId>,
) -> Result<()> {
...
pub async fn pin_route(
    ...
    let rows: Vec<CountRow> = {
        let raw: Vec<surrealdb::types::Value> = count_res
            .take(0)
            .map_err(|e| AppError::Internal(e.to_string()))?;
        crate::db::queries::rows_to(raw).map_err(|e| AppError::Internal(e.to_string()))?
    };
