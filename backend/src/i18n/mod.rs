...
impl I18n {
    ...
    pub fn new() -> Self {
        ...
        ja_bundle.add_resource(ja_resource).expect("Failed to add Japanese resource");
    ...
    pub fn translate_with_args(
        ...
        let bundle = self.bundles.get(locale).unwrap_or_else(|| {
            self.bundles.get(DEFAULT_LOCALE).expect("Default locale must exist")
        });
    ...
    pub fn select_locale(&self, accept_language: Option<&str>) -> &str {
        ...
        for part in header.split(',') {
            ...
            if self.bundles.contains_key(lang) {
                return SUPPORTED_LOCALES.iter().find(|&&l| l == lang).copied().unwrap_or(DEFAULT_LOCALE);
            }
            // 部分一致チェック (例: "en" -> "en-US")
            for &supported in SUPPORTED_LOCALES {
                if supported.starts_with(lang) || lang.starts_with(supported.split('-').next().unwrap_or(supported)) {
                    return supported;
                }
            }
        }
...
macro_rules! t {
    ($key:expr) => {
        $crate::i18n::I18N.translate($crate::i18n::DEFAULT_LOCALE, $key)
    };
    ($key:expr, $locale:expr) => {
        $crate::i18n::I18N.translate($locale, $key)
    };
}
...
macro_rules! t_args {
    ($key:expr, $($name:ident = $value:expr),*) => {{
        ...
        $crate::i18n::I18N.translate_with_args($crate::i18n::DEFAULT_LOCALE, $key, Some(&args))
    }};
    ($key:expr, $locale:expr, $($name:ident = $value:expr),*) => {{
        let mut args = fluent::FluentArgs::new();
        $(args.set(stringify!($name), $value);)*
        $crate::i18n::I18N.translate_with_args($locale, $key, Some(&args))
    }};
}
