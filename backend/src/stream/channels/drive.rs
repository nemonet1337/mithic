//! Drive Channel
//!
//! Real-time drive file change notifications.
...
impl DriveChannel {
    pub fn new(id: String, user_id: Option<String>) -> Self {
        Self {
            base: Arc::new(Mutex::new(ChannelBase::new(id, "drive"))),
            user_id,
...
impl Channel for DriveChannel {
    ...
    async fn init(&self, redis: &DragonflyClient) -> anyhow::Result<()> {
        ...
        let handle = tokio::spawn(async move {
            ...
            let channel = format!("drive:{}", user_id);
            ...
            while let Some(msg) = stream.next().await {
                ...
                if let Ok(data) = serde_json::from_str::<serde_json::Value>(&payload) {
                    let event_type = data.get("type").and_then(|v| v.as_str()).unwrap_or("fileCreated");
        ...
        *sub = Some(handle);
        ...
        info!("Drive channel initialized for user {}", user_id);
        Ok(())
    }
