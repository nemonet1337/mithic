...
static ITALIC_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"<i>(.*?)</i>|(?<!\*)\*([^*]+)\*(?!\*)").unwrap()
});

static STRIKE_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"~~(.+?)~~").unwrap()
});

static CENTER_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"<center>(.*?)</center>").unwrap()
});

static SMALL_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"<small>(.*?)</small>").unwrap()
});
...
static MATH_INLINE_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\\\((.+?)\\\)").unwrap()
});

static MATH_BLOCK_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\\\[([^\]]+)\\\]").unwrap()
});

static SEARCH_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(.+?)\s+検索").unwrap()
});
...
fn try_match_patterns(text: &str) -> Option<(MfmNode, usize)> {
    ...
    if let Some(mat) = CODE_BLOCK_REGEX.find(text) {
        if mat.start() == 0 {
            let caps = CODE_BLOCK_REGEX.captures(&text[mat.start()..mat.end()]).unwrap();
    ...
    if let Some(mat) = BOLD_REGEX.find(text) {
        if mat.start() == 0 {
            ...
            let content = caps.get(1).or_else(|| caps.get(2))
