...
メディアは `STORAGE_TYPE=local`（`media_files`）か `s3`。RustFS は `STORAGE_TYPE=s3` と `STORAGE_S3_ENDPOINT` で指す。結合テストは `cargo test -p mithic-server --lib rustfs_put_get -- --ignored`。
...
| `STORAGE_TYPE` | `local`（既定）または `s3`。それ以外は起動しない |
